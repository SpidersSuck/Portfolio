...
export function Navigation({ currentPage, onPageChange, onLoginClick, onLogoutClick }: NavigationProps) {
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const { user } = useAuth();

  // Check if user is admin (you can customize this check based on your auth system)
  const isAdmin = user?.email === 'admin@aww.com';
  ...
  if (isAdmin) {
    navigationItems.push({ id: 'edit-events', label: '⚙️ Admin' });
  }
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white shadow-md">
      ...
      {isAdmin && (
        <motion.div
          initial={{ y: -100 }}
          animate={{ y: 0 }}
          className="bg-gradient-to-r from-[#004080] to-[#0056b3] text-white text-center py-1 text-sm font-medium"
        >
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {user ? (
              <Button 
                ...
                variant="outline"
                className="border-[#f7941D] text-[#f7941D] hover:bg-[#f7941D] hover:text-white px-6 py-2 rounded-full transition-all duration-200 hover:scale-105"
              >
              ...
              <Button 
                ...
                className="bg-[#f7941D] hover:bg-[#F79520] text-white px-6 py-2 rounded-full transition-all duration-200 hover:scale-105"
              >
            ...
            <div className="px-2 pt-2 pb-3 space-y-1">
              ...
              {user ? (
                <Button 
                  ...
                  variant="outline"
                  className="w-full mt-2 border-[#f7941D] text-[#f7941D] hover:bg-[#f7941D] hover:text-white"
                >
                  <LogOut size={16} className="mr-2" />
                  Logout
                </Button>
                ...
                <Button 
                  ...
                  className="w-full mt-2 bg-[#f7941D] hover:bg-[#F79520] text-white"
                >
                  <LogIn size={16} className="mr-2" />
                  Login
                </Button>
